...
Example of a Dockerized python application with GPU acceleration.
...
- [NVIDIA Driver (matching GPU card)](https://www.nvidia.com/drivers/)
- [NVIDIA Container
  Toolkit (any recent version should work)](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html)

## Instructions

Start by cloning this repository:
- `git clone git@github.com:PoPETS-AEC/example-docker-python-gpu.git` (SSH)
- `git clone https://github.com/PoPETS-AEC/example-docker-python-gpu.git` (HTTPS)
...
> ```bash
> docker pull ghcr.io/popets-aec/example-docker-python-gpu:main
> ```
...
> to be that volume, expose all GPUs, and provide an interactive bash terminal:
> ```bash
> docker run --rm -it -v ${PWD}:/workspaces/example-docker-python-gpu \
>     -w /workspaces/example-docker-python-gpu \
>     --gpus all \
>     --entrypoint bash ghcr.io/popets-aec/example-docker-python-gpu:main
> ```
...
> ```bash
> docker build -t example-docker-python-gpu:main .
> ```
> 2. Launch the Docker container, attach the current working directory (i.e.,
> run from the root of the cloned git repository) as a volume, set the context
> to be that volume, expose all GPUs, and provide an interactive bash terminal:
...
>     -w /workspaces/example-docker-python-gpu \
>     --gpus all \
>     --entrypoint bash example-docker-python-gpu:main
> ```
> 3. Execute the example script:
> ```bash
> ./test.sh
